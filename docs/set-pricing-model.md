...
- The discount is **scoped to a set**: a product's discount only applies *because
  it is in a set*, not because it is a product in general. (A product with no set
...
- Resolution is **per product, independently**: for each basket item we find the
  ...
  *not* pick one set "for the basket" and apply it uniformly.

## 2. Why this shape

- **Percent, per product** — a set is a bundle of *different* products at
  *different* base prices; a single flat "set price" (as the old Ovis Felszerelés
...
- **Biggest-discount wins (no stacking)** — a product in two sets must not be
  ...
  also always optimal *for that product* under an additive percent model, and it
...
- **Per-product, not per-basket** — it degrades gracefully: if the buyer drops one
  set member, the *other* products simply keep (or lose) their discount based on
...
i.e. on the `{ product, discount_percent }` entry inside a group — *not* on the
product and *not* as a single number on the group.
...
- `calculatePriceForItem(product)` already ends with
  ...
  feeds a *different* `discountMultiplier` (from the winning set) into that same
...
3. **Buyer-facing label** — when a set discount wins, do we show *"Szett
   kedvezmény"* (distinct from the generic *"Kedvezmény"*) in the price breakdown?
