...
- [x] 2.3 Recompute per-item `active` / `pending-partner` / `pending-material`
      (+`canSync`) statuses from the new allocation and instances — verify the
      pending-state scenarios as tests
- [x] 2.4 Update `resolveActiveSetDiscount(s)` and `calculatePriceForItem` to
...
- [x] 3.1 Compute instances once in `CheckoutForm.svelte` and render a "set
...
- [x] 3.2 Remove the per-line discount label/props from `CheckoutItem.svelte`
      and refocus `CheckoutDeals.svelte` on pending opportunities + the active
      instance list; verify `npm run check`
- [x] 3.3 Re-express `SetPanel.svelte` / `ProductOrder.svelte` set status against
      instances, keeping "add related" candidates and pending-partner /
      pending-material (one-click sync); verify on a product page

## 4. Submission

- [x] 4.1 In `orderSubmit.ts`, drop the per-item discount-source line and add a
      basket-level set-discount summary derived from the instances; verify the
      order-submit formatter tests

## 5. Verify end to end

- [x] 5.1 Run `npm run check`, `npm test`, `npm run lint`, `npm run lint:style`
      — all green with the rewritten expectations
- [x] 5.2 Build and run `npm run test:hydration` to confirm the order/checkout
      islands hydrate and price identically to the allocation
- [x] 5.3 Run `npx openspec validate rearchitect-set-discounts --strict` and fix
