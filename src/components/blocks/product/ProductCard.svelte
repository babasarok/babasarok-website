...
<a {href} class="group flex h-full w-32 flex-col items-center sm:items-start lg:w-50">
  {#if image}
    <img
      src={image.src}
      ...
      {...image.attributes}
      alt={title}
      class="aspect-square w-full rounded-[20px] object-cover transition-all duration-300 group-hover:shadow-[0_20px_40px_0_rgba(0,0,0,0.12)]"
    />
  {/if}
  {#if category}
