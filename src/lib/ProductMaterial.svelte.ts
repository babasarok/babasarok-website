...
export interface IProductMaterial extends Required<
  Omit<TinaResolvedProductMaterial, "color_count">
> {
...
export class ProductMaterial implements IProductMaterial {
  ...
  constructor(material: TinaResolvedProductMaterial, product: Pick<IProduct, "fields">) {
    ...
    this.color_count = $derived.by(() => {
      ...
      const current = product.fields?.find((f) => f.name === this.original_color_count)?.value
        ?.value;
      if (!current) {
        return undefined;
      }
